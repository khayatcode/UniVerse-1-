  ...
  <div>
      <nav className='navbar navbar-expand-xl p-4 fixed-top' style={{ backgroundImage: `url(${MilkyWay})`}}>
        <div className='container d-flex justify-content-between'>
            <button className='navbar-brand display-1 text-white' style={{ fontSize: '38px', marginLeft: '1rem', backgroundColor: 'transparent', border: 'none' }} onClick={(e) => { navigateToDashboard(sessionId); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>UniVerse</button>          <button className='navbar-toggler' type='button' data-bs-toggle='collapse' data-bs-target='#navbarNav' aria-controls='navbarNav' aria-expanded='false' aria-label='Toggle navigation'>
